  ...
  useEffect(() => {
    const fetchPoems = async () => {
      ...
      try {
        ...
        if (data) {
          console.log('DATA FROM API SIDEBAR', data)
          setPageIndex((prev) => {
          ...
          const poems = data.data.map(({ id, attributes }) => ({
            ...
            creatorOfPoem: attributes.users_poem_creator?.data?.attributes?.username || '',
          }))
          setPoem(poems)
          setLoading(false)
          return data
    ...
    <>
      {Loading ? (
        <div className="h-full w-full flex flex-col justify-center items-center">
          ...
          <h4 className="mt-2 text-indigo-600 font-lg">Loading poems ...</h4>
        </div>
      ) : (
        <>
          {/* <div className="h-full w-full flex flex-col justify-center items-center"> */}
          <div className="h-full w-full">
            {poem.map((poemData, i) => {
              ...
              return (
                <div
                  className="bg-white rounded-lg shadow-sm overflow-hidden w-full mt-3"
                  key={id}
                >
                  ...
                  <div className="px-4 py-3">
                    ...
                    <p className="text-gray-800 text-sm">{userPoem}</p>
                  </div>
